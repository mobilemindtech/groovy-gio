...
class Monoids {
    ...
    private static abstract class BaseMonoid<A> implements Monoid {
        ...
        @Override
        Monoid<A> append(Monoid other) { asMonoid(val + other.get()) }
    ...
    private static Monoid standardMonoid(obj) {
        new BaseMonoid(obj) {
            @Override
            Monoid getIdentity() { asMonoid(obj.getClass().newInstance()) }
        }
    }
    ...
    static Monoid<String> asMonoid(String s) {
        new BaseMonoid<String>(s) {
            @Override
            Monoid<String> getIdentity() { asMonoid('') }
        }
    }
    ...
    static Monoid<Number> asMonoidSum(Number n) {
        new BaseMonoid<Number>(n) {
            @Override
            Monoid<Number> getIdentity() { asMonoid(0) }

            @Override
            Monoid<Number> append(Monoid other) { asMonoid(n + other.get()) }
        }
    }
    ...
    static Monoid<Number> asMonoidProd(Number n) {
        new BaseMonoid<Number>(n) {
            @Override
            Monoid<Number> getIdentity() { asMonoid(1) }
    ...
    static Monoid<Boolean> asMonoidAny(Boolean b) {
        new BaseMonoid<Boolean>(b) {
            @Override
            Monoid<Boolean> getIdentity() { asMonoid(false) }

            @Override
            Monoid<Boolean> append(Monoid other) { asMonoid(b || other.get()) }
        }
    }
    ...
    static Monoid<Boolean> asMonoidAll(Boolean b) {
        new BaseMonoid<Boolean>(b) {
            @Override
            Monoid<Boolean> getIdentity() { asMonoid(true) }
    ...
    static Monoid<Option> asMonoid(Option o) {
        ...
        self = new BaseMonoid<Option>(o) {
            @Override
            Monoid<Option> getIdentity() { asMonoid((Option) Option.ofNone()) }

            @Override
            Monoid<Option> append(Monoid other) {
                if (o.isNone())
                    other
